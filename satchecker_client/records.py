...
stops (see :mod:`satchecker_client.client` for the handover date), so any caller
spanning the handover has to carry both formats at once. Rather than thread a format flag through the
...
There is a third loss too — :func:`satchecker_client.tle_parse.tle_epoch_jd`
...
:func:`~satchecker_client.tle_parse.validate_elements` range and finiteness
